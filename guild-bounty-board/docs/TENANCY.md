...
**Public routes (same on every hack — swap env per deploy):** collaborator-themed landing **`/credits/`** (alias **`/credits/hackathon`** — redirects to **`/`**); OG Cursor **`/credits/redeem`**, **`/credits/submit`**, **`/credits/judge`**, **`/credits/manager`** (guild board embedded). Credits slug: **`NEXT_PUBLIC_CREDITS_FIRESTORE_PROJECT_SLUG`**. Landing copy: **`NEXT_PUBLIC_HACKATHON_*`**. Optional Thrads chrome on **`/`** only: **`NEXT_PUBLIC_HACKATHON_SKIN=thrad`** (or `"skin":"thrad"` inside **`NEXT_PUBLIC_HACKATHON_LANDING_JSON`**). Legacy **`/credits/event/[slug]/redeem`** → **`/credits/redeem`**.
